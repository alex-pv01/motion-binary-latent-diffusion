...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, texts_path, motions_path, mean, std, split_file, w_vectorizer):
        # self.opt = opt
        self.w_vectorizer = w_vectorizer
        self.max_length = 20
        self.pointer = 0
        ...
        for name in tqdm(id_list):
            try:
                # print(pjoin(self.motions_path, name + '.npy'))
                motion = np.load(pjoin(self.motions_path, name + '.npy'))
                if (len(motion)) < min_motion_len or (len(motion) >= 200):
                    continue
                text_data = []
                flag = False
                with cs.open(pjoin(self.texts_path, name + '.txt')) as f:
                    for line in f.readlines():
                        text_dict = {}
                        line_split = line.strip().split('#')
                        caption = line_split[0]
...
class TextOnlyDataset(data.Dataset):
    def __init__(self, texts_path, mean, std, split_file):
        self.mean = mean
        self.std = std
        # self.opt = opt
        self.data_dict = []
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train",
                 motions_path=None, texts_path=None, **kwargs):
        self.mode = mode
        
        self.dataset_name = 't2m'
        self.dataname = 't2m'

        meta_dir = '/home/apujol/mbld/dataset'
        data_root = '/home/apujol/mbld/dataset/HumanML3D'
        ...
        if mode == 'gt':
            ...
            self.mean = np.load(pjoin(meta_dir, f'{self.dataset_name}_mean.npy'))
            self.std = np.load(pjoin(meta_dir, f'{self.dataset_name}_std.npy'))
        elif mode in ['train', 'eval', 'text_only']:
            # used by our models
            self.mean = np.load(pjoin(data_root, 'Mean.npy'))
            self.std = np.load(pjoin(data_root, 'Std.npy'))

        if mode == 'eval':
            # used by T2M models (including evaluators)
            # this is to translate their norms to ours
            self.mean_for_eval = np.load(pjoin(meta_dir, f'{self.dataset_name}_mean.npy'))
            self.std_for_eval = np.load(pjoin(meta_dir, f'{self.dataset_name}_std.npy'))

        self.split_file = pjoin(data_root, f'{split}.txt')
        if mode == 'text_only':
            self.t2m_dataset = TextOnlyDataset(texts_path, self.mean, self.std, self.split_file)
        else:
            # self.w_vectorizer = WordVectorizer(pjoin(abs_base_path, 'glove'), 'our_vab')
            self.w_vectorizer = WordVectorizer(GLOVE_PATH, 'our_vab')
            self.t2m_dataset = Text2MotionDatasetV2(texts_path, motions_path, self.mean, self.std, self.split_file, self.w_vectorizer)
