...
def draw_to_batch_smplh(smpl_joints_batch, kinematic_chain, title_batch=None, outname=None, log_dir=None, step=None):

    batch_size = len(smpl_joints_batch)
    out = []
    for i in tqdm(range(batch_size), desc='Drawing', total=batch_size):
        ...
        if outname is not None:
            if log_dir is not None:
                if step is not None:
                    out_path = log_dir + '/' + f'{step:09}_{outname[i]}.gif'
                else:
                    out_path = log_dir + '/' + outname[i] + '.gif'
            else:
                out_path = outname[i] + '.gif'
            print('Saving to', out_path)
            imageio.mimsave(out_path, np.array(out[-1]), fps=20)
    out = torch.stack(out, axis=0)
    return out
...
if __name__ == '__main__':
    ...
    example_path = "/home/apujol/mbld/datasets/MotionX/MotionX/datasets/motion_data/new_joints/idea400/subset_0052/Bending_To_Pick_Up_And_Walking_At_The_Same_Time.npy"
